...
```{r, message=F,warning=F}
z_ultimate <- glmer(hits ~ z_mem * za *zrt*zv + (1+za+zv+zrt+z_mem | Participant.Private.ID),mem_att,family="binomial",control=glmerControl())
...
z2 <- glmer(hits ~ z_mem * za * zv *zrt+ (1 +z_mem+zrt+zv+za| Participant.Private.ID),mem_att_clean9,family="binomial",control=glmerControl())
#tab_model(z2, show.ci = FALSE,show.re.var=FALSE,show.icc=FALSE,show.obs=FALSE)
...
group_averaged_emo1 <- emo_mematt %>%
  ...
  select(Img,Participant.Private.ID,hits,emorate1)%>%
  ungroup()%>%
  mutate(zv = scale(emorate1))
...
group_averaged_emo2 <- emo_mematt %>%
  ...
  select(Img, Participant.Private.ID,zrt,emorate2) %>%
  ungroup()%>%
  mutate(za = scale(emorate2))
...
sust <- rbind(delay_sust1,delay_sust2,delay_sust3,delay_sust4,delay_sust5,delay_sust6,delay_sust7,delay_sust8,delay_sust9,delay_sust10)
memory <- rbind(delay_mem1,delay_mem2,delay_mem_early2,delay_mem3,delay_mem4,delay_mem5,delay_mem6,delay_mem7,delay_mem8,delay_mem9,delay_mem10)
emo <- rbind(delay_emo1,delay_emo2,delay_emo3,delay_emo4,delay_emo5,delay_emo6,delay_emo7,delay_emo8,delay_emo_early2,delay_emo9,delay_emo10)
...
sust_full_rt <-sust_full_rt %>%
  ...
  filter(`Participant Private ID` !="11638956" & `Participant Private ID` !="11658165" & `Participant Private ID` !="11682062"& `Participant Private ID` !="11638973" & `Participant Private ID` !="11682065" & `Participant Private ID` !="11758423" 
)
