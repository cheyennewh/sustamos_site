...
delay_sust <- rbind(delay_sust1,delay_sust2,delay_sust3,delay_sust4,delay_sust5,delay_sust6,delay_sust7,delay_sust8,delay_sust9,delay_sust10,delay_sust11,delay_sust12)
delay_memory <- rbind(delay_mem1,delay_mem2,delay_mem_early2,delay_mem3,delay_mem4,delay_mem5,delay_mem6,delay_mem7,delay_mem8,delay_mem9,delay_mem10,delay_mem11,delay_mem12)
delay_emo <- rbind(delay_emo1,delay_emo2,delay_emo3,delay_emo4,delay_emo5,delay_emo6,delay_emo7,delay_emo8,delay_emo_early2,delay_emo9,delay_emo10,delay_emo11,delay_emo12)
...
# Fit the GLMM model
delay_model <- glmer(hits ~ zrt + (1 | `Participant Private ID`), family = binomial, data = delay_mem_att)
...
delay_pred_data <- delay_mem_att_clean %>%
  # Create a new data frame for prediction with original values
  mutate(
    pred = predict(delay_model, type = "response"),
    se = predict(delay_model, type = "link", se.fit = TRUE)$se.fit
  )

# Calculate confidence intervals
delay_pred_data <- delay_pred_data %>%
  mutate(
    lwr = pred - 1.96 * se,
    upr = pred + 1.96 * se
  )
...
```{r,message=F,warning=F}
delay_model2 <- glmer(hits ~ z_mem + (1+z_mem| `Participant Private ID`), family = binomial, data = delay_mem_att,control = glmerControl())
delay_mem_att_clean2 <- delay_mem_att %>%
  filter(!is.na(z_mem) & !is.na(hits))
# Create a data frame with predictions
delay_pred_data2 <- delay_mem_att_clean2 %>%
  # Create a new data frame for prediction with original values
  mutate(
    pred = predict(delay_model2, type = "response"),
    se = predict(delay_model2, type = "link", se.fit = TRUE)$se.fit
...
ggplot(delay_pred_data2, aes(x = z_mem, y = hits)) +
  geom_point() +
  geom_line(aes(y = pred, color = as.factor(`Participant Private ID`))) + 
  geom_smooth(aes(y = pred), color = "black") + # Predicted line 
  theme_ipsum(axis_title_just = "cc", axis_title_size = 12, axis_text_size = 9) +
...
delay_model3 <- glmer(hits ~ zv + (1+zv| `Participant Private ID`), family = binomial, data = delay_mem_att,control = glmerControl())
...
ggplot(delay_pred_data3, aes(x = zv, y = hits)) +
  geom_point() +
  geom_line(aes(y = pred, color = as.factor(`Participant Private ID`))) + 
  geom_smooth(aes(y = pred), color = "black") + # Predicted line
  theme_ipsum(axis_title_just = "cc", axis_title_size = 12, axis_text_size = 9) +
...
## How might things change when we just analyze data for participants in the lower half of memorory performance?
...
delay_model3 <- glmer(hits ~ zv + (1+zv| `Participant Private ID`), family = binomial, data = delay_bad_data,control = glmerControl())
delay_mem_att_clean3 <- delay_bad_data %>%
  filter(!is.na(zv) & !is.na(hits))
# Create a data frame with predictions
delay_pred_data3 <- delay_mem_att_clean3 %>%
  # Create a new data frame for prediction with original values
...
delay_model6 <- glmer(hits ~ za + (1+za| `Participant Private ID`), family = binomial, data = delay_bad_data,control = glmerControl())
delay_mem_att_clean6 <- delay_bad_data %>%
  filter(!is.na(za) & !is.na(hits))
# Create a data frame with predictions
delay_pred_data6 <- delay_mem_att_clean6 %>%
  # Create a new data frame for prediction with original values
...
delay_model2 <- glmer(hits ~ z_mem + (1+z_mem| `Participant Private ID`), family = binomial, data = delay_bad_data,control = glmerControl())
delay_mem_att_clean2 <- delay_bad_data %>%
  filter(!is.na(z_mem) & !is.na(hits))
# Create a data frame with predictions
delay_pred_data2 <- delay_mem_att_clean2 %>%
  # Create a new data frame for prediction with original values
...
# Fit the GLMM model
delay_model2 <- glmer(hits ~ zrt + (1 +zrt| `Participant Private ID`), family = binomial, data = delay_bad_data, control = glmerControl())

delay_mem_att_clean <-delay_bad_data %>%
  filter(!is.na(zrt) & !is.na(hits))
...
## How might these relationships change when looking at just top memory performers?
...
delay_good_data <- delay_mem_att[delay_mem_att$`Participant Private ID` %in% delay_good_mems,]


delay_model3 <- glmer(hits ~ zv + (1+zv| `Participant Private ID`), family = binomial, data = delay_good_data,control = glmerControl())
delay_mem_att_clean3 <- delay_good_data %>%
  filter(!is.na(zv) & !is.na(hits))
# Create a data frame with predictions
delay_pred_data3 <- delay_mem_att_clean3 %>%
  # Create a new data frame for prediction with original values
...
edelay_mo_group_ar <-delay_emo_group %>%
  filter(aff_type=="arousal")%>%
  group_by(group)%>%
  mutate(group_scale_ar = scale(z2))%>%
  ungroup()

delay_model6 <- glmer(hits ~ za + (1+za| `Participant Private ID`), family = binomial, data = delay_good_data,control = glmerControl())
delay_mem_att_clean6 <- delay_good_data %>%
  filter(!is.na(za) & !is.na(hits))
# Create a data frame with predictions
delay_pred_data6 <-delay_mem_att_clean6 %>%
  # Create a new data frame for prediction with original values
...
```{r,message=F,warning=F}
delay_model2 <- glmer(hits ~ z_mem + (1+z_mem| `Participant Private ID`), family = binomial, data = delay_good_data,control = glmerControl())
delay_mem_att_clean2 <-delay_good_data %>%
  filter(!is.na(z_mem) & !is.na(hits))
# Create a data frame with predictions
delay_pred_data2 <- delay_mem_att_clean2 %>%
  # Create a new data frame for prediction with original values
...
# Fit the GLMM model
delay_model2 <- glmer(hits ~ zrt + (1 +zrt| `Participant Private ID`), family = binomial, data = delay_good_data, control = glmerControl())

delay_mem_att_clean <- delay_good_data %>%
  filter(!is.na(zrt) & !is.na(hits))
...
## How might these factors cumulatively influence memory?

### Additive model

```{r ,message=F,warning=F}
delay_model10 <- glmer(hits ~ zrt + zv + za +z_mem+ (1 +zrt+zv+za+z_mem| `Participant Private ID`), family = binomial, data = delay_mem_att, control = glmerControl())
```

```{r,message=F,warning=F}
model_summary <- summary(delay_model10)
...
# Create the plot
ggplot(delay_mem_rt1, aes(x = `Trial Number`, y = `Reaction Time`, color=`Participant Private ID`, alpha=.6)) +
  geom_line() +   # Add points for individual trials
...
ggplot(delay_mem_rt1, aes(x = `Trial Number`, y = `Reaction Time`, color=`Participant Private ID`, alpha=.6)) +
  ...
  ylim(0,10000)+
  geom_hline(yintercept= 2380.765)
ggplot(delay_mem_rt2, aes(x = `Trial Number`, y = `Reaction Time`, color=`Participant Private ID`, alpha=.6)) +
  geom_line() +  ## Add points for individual trials
  labs(title = "Participant Response Times Over the Course of the Experiment",
       x = "Trial Number",
       y = "Response Time (ms)") +
...
ggplot(delay_mem_rt2, aes(x = `Trial Number`, y = `Reaction Time`, color=`Participant Private ID`, alpha=.6)) +
  ...
  ylim(0,10000)+
  geom_hline(yintercept= 2380.765)
ggplot(delay_mem_rt3, aes(x = `Trial Number`, y = `Reaction Time`, color=`Participant Private ID`, alpha=.6)) +
  geom_line() +  ## Add points for individual trials
  labs(title = "Participant Response Times Over the Course of the Experiment",
       x = "Trial Number",
       y = "Response Time (ms)") +
...
ggplot(delay_mem_rt3, aes(x = `Trial Number`, y = `Reaction Time`, color=`Participant Private ID`, alpha=.6)) +
  ...
  ylim(0,10000)+
  geom_hline(yintercept= 2380.765)
ggplot(delay_mem_rt4, aes(x = `Trial Number`, y = `Reaction Time`, color=`Participant Private ID`, alpha=.6)) +
  geom_line() +  ## Add points for individual trials
  labs(title = "Participant Response Times Over the Course of the Experiment",
       x = "Trial Number",
       y = "Response Time (ms)") +
...
delay_mem_rt$`Participant Private ID` <- as.numeric(delay_mem_rt$`Participant Private ID`)
