...
z2 <- glmer(hits ~ z_mem * za * zv + (1 | Participant.Private.ID),mem_att_clean9,family="binomial",control=glmerControl())
#tab_model(z2, show.ci = FALSE,show.re.var=FALSE,show.icc=FALSE,show.obs=FALSE)
...
A 3-way version of thisplot is also available.Data was separated into equally sized negative, positive, or neutral valence bins before predictions were made using the regression.The predicted memory for each image is plotted against the memorability of the image. The line colors represent the arousal groups (high/low). The plots below show the interaction between memorability and arousal influences on memory predictions for each valence bin.

<details>
<summary> 3-way plots </summary>
...
```{r, message=F,warning=FALSE}
neg <- glmer(hits ~ z_mem *  za *zv +(1|Participant.Private.ID),low_v,family="binomial")
tab_model(neg, show.ci = FALSE,show.re.var=FALSE,show.icc=FALSE,show.obs=FALSE)
```
</details>
...
<summary> Model details </summary>
```{r, message=F,warning=FALSE}
  neut  <- glmer(hits ~ z_mem * za*zv +(1|Participant.Private.ID),mid_v,family="binomial")
tab_model(  neut, show.ci = FALSE,show.re.var=FALSE,show.icc=FALSE,show.obs=FALSE)
...
<summary> Model details </summary>
```{r, message=F,warning=FALSE}
positive <- glmer(hits ~ z_mem *  za*zv +(1|Participant.Private.ID),high_v,family="binomial")
...
tab_model(both_slope, show.ci = FALSE,show.re.var=FALSE,show.icc=FALSE,show.obs=FALSE)
...
An alternative version of the above models was run.  Note that the model formula for this set is as follows: $\text{hits} \sim \text{Memorability} \cdot \text{Arousal} \sim \text{Valence}+ \cdot \text{Reaction Time}(1 \mid \text{Participant.Private.ID}), \text{family=binomial}$. The 4 way interactive model accounts for the effects of all memory predictors and their interactions.


```{r, message=F,warning=FALSE}
pred_data10 <- mem_att_clean9 %>%
  mutate(
    ar = factor(ifelse(mem_att_clean9$Mean_Arousal>4.48,"high_ar","low_ar"), labels = c("High Arousal", "Low Arousal"))
)
  
pred_data10$`val_groups`<- cut_number(pred_data10$Mean_Valence ,3)

z3 <- glmer(hits ~ z_mem * za *zv*zrt + (1 | Participant.Private.ID),mem_att_clean9,family="binomial",control=glmerControl())
#tab_model(z2, show.ci = FALSE,show.re.var=FALSE,show.icc=FALSE,show.obs=FALSE)

pred_data10 <- pred_data10 %>%
  mutate(
    pred2 = predict(z2, type = "response")
  )

ggplot(pred_data10, aes(x = Memorability_Score, y = hits, color=ar)) +
  binomial_smooth(aes(y = pred2,  color = ar),se=F, alpha=.8, size= .7) + 
  theme_ipsum(axis_title_just = "cc", axis_title_size = 12, axis_text_size = 9) +
  ylim(0,1) + 
  xlab("Memorability") +
...
high_v2 <- pred_data10[pred_data10$val_groups=="(5.95,7.07]",]
mid_v2 <- pred_data10[pred_data10$val_groups=="(2.98,5.95]",]
low_v2 <- pred_data10[pred_data10$val_groups=="[1.49,2.98]",]


 p4<- ggplot(low_v, aes(x =  Memorability_Score, y = hits, color=ar)) +
  binomial_smooth(aes(y =  pred2,  color = ar),se=F, alpha=.8, size= .7) + 
  theme_ipsum(axis_title_just = "cc", axis_title_size = 12, axis_text_size = 9) +
  ylim(0,1) + 
  xlab("Memorability") +
...
p5 <- ggplot(mid_v, aes(x =  Memorability_Score, y = hits, color=ar)) +
  binomial_smooth(aes(y = pred2,  color = ar),se=F, alpha=.8, size= .7) + 
  theme_ipsum(axis_title_just = "cc", axis_title_size = 12, axis_text_size = 9) +
  ylim(0,1) + 
  xlab("Memorability") +
 ...
 p6 <- ggplot(high_v, aes(x =  Memorability_Score, y = hits, color=ar)) +
  binomial_smooth(aes(y =  pred2,  color = ar),se=F, alpha=.8, size= .7) + 
  theme_ipsum(axis_title_just = "cc", axis_title_size = 12, axis_text_size = 9) +
  xlab("Memorability") +
  ylab("HIT?") +
...
<summary> Model details </summary>
```{r, message=F,warning=FALSE}
neg2 <- glmer(hits ~ z_mem *  za *zv*zrt+(1|Participant.Private.ID),low_v2,family="binomial")
tab_model(neg2, show.ci = FALSE,show.re.var=FALSE,show.icc=FALSE,show.obs=FALSE)
```
</details>
...
<summary> Model details </summary>
```{r, message=F,warning=FALSE}
  neut2  <- glmer(hits ~ z_mem * za*zv*zrt +(1|Participant.Private.ID),mid_v2,family="binomial")
tab_model(  neut2, show.ci = FALSE,show.re.var=FALSE,show.icc=FALSE,show.obs=FALSE)
...
<summary> Model details </summary>
```{r, message=F,warning=FALSE}
posit2 <- glmer(hits ~ z_mem *  za*zv*zrt +(1|Participant.Private.ID),high_v2,family="binomial")
tab_model(posit2, show.ci = FALSE,show.re.var=FALSE,show.icc=FALSE,show.obs=FALSE)
```
</details>
</details>
...
group_averaged_emo1 <- emo_mematt %>%
  ...
  ungroup()%>%
  select(Img, zv,Participant.Private.ID,hits)
group_averaged_emo2 <- emo_mematt %>%
  ...
  mutate(
    ...
    za = scale(emorate2) )%>%
  ungroup()%>%
  select(Img, za,Participant.Private.ID,zrt)
...
z  <- glmer(hits ~ z_mem * za*zv*zrt +(1+za+zv+zrt|Participant.Private.ID),individ_emo,family="binomial")
tab_model(z, show.ci = FALSE,show.re.var=FALSE,show.icc=FALSE,show.obs=FALSE)
...
<details>
<Summary> Models using group-averaged valence and arousal ratings </summary>
...
```{r, message=F,warning=F }
  #individ_emo$Participant.Private.ID <- as.factor(individ_emo$Participant.Private.ID)
z  <- glmer(hits ~ z_mem * za*zv*zrt +(1+za+zv+zrt|Participant.Private.ID),group_av_emo,family="binomial")
tab_model(z, show.ci = FALSE,show.re.var=FALSE,show.icc=FALSE,show.obs=FALSE)
